...
namespace SmartPM.Services
{
    public static class TimesheetService
    {
        ...
        public static async Task<string> ListConfirm(string token, string uid)
        {
            try
            {
                // This is the postdata
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.Timeout = new TimeSpan(0, 0, 15);
                    using (var response = await client.PostAsync("http://192.168.88.200:56086/APIRest3/listConfirmTimesheet", content))
        ...
        public static async Task<string> actionConfirm(string timeNumber,string pid, string tid, string fid, string actid, string uid)
        {
            try
            {
                // This is the postdata
                var postData = new List<KeyValuePair<string, string>>(2);
                postData.Add(new KeyValuePair<string, string>("timeNumber", timeNumber));
                postData.Add(new KeyValuePair<string, string>("pid", pid));
                postData.Add(new KeyValuePair<string, string>("tid", tid));
                postData.Add(new KeyValuePair<string, string>("fid", fid));
                postData.Add(new KeyValuePair<string, string>("actid", actid));
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.Timeout = new TimeSpan(0, 0, 15);
                    using (var response = await client.PostAsync("http://192.168.88.200:56086/APIRest3/ConfirmAction", content))
        ...
        public static async Task<string> reqTimesheet(string uid)
        {
            try
            {
                // This is the postdata
                ...
                {
                    client.Timeout = new TimeSpan(0, 0, 15);
                    using (var response = await client.PostAsync("http://192.168.88.200:56086/APIRest3/listTimesheet", content))
                    {
                        if (((int)response.StatusCode >= 200) && ((int)response.StatusCode <= 299))
                        {
                            using (var responseContent = response.Content)
